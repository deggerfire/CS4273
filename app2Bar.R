...
server <- function(input, output) {
  ...
  groupLtrigger <- function(){
    if(input$Data_Set == "Calls for Service")
    {
      ...
      # Table the data and convert it to a data frame
      tabulatedData <- as.data.frame(table(data$PoliceCallPriority))
      # Sort the data by decreasing frequency and select the top 5
      top5Data <- tabulatedData %>%
        arrange(desc(Freq)) %>%
        head(5)
      # Update the outputBarPlot call to use the processed top 5 data
      outputBarPlot(top5Data, legend = "Call Priority", xlab = "Call Priority", ylab = "Amount")
    }
  }
  }
