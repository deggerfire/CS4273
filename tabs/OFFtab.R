...
OFF_tab <- function(){
  ...
  tab <- tabItem(tabName = "OFF",
    ...
    tabBox(
      ...
      OFF_Plot("TAB 1", "OFF_table_1", OFF_selectors[1]),
      OFF_Plot("TAB 2", "OFF_table_2", OFF_selectors[2])
    ),
    # Makes the second graph area
    tabBox(
      height = "500px",
      # Uses functions to make what is in each tab (string is the name of the plotOutput)
      OFF_Plot("TAB 3", "OFF_table_3", OFF_selectors[3]),
      OFF_Plot("TAB 4", "OFF_table_4", OFF_selectors[4])
    )
  )
  return(tab)
}
...
OFF_Plot <- function(tabName, plotName, widgetName){
...
## !!TODO: not working!! ##
OFF_populate_Widgets <-function(session, input, data){
