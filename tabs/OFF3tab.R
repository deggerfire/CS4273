...
#     goes from upper left to lower right order
OFF3_populate_Widgets <-function(session, Graph1_selector, Graph2_selector, Graph3_selector, Graph4_selector, Topbar_selector1){
  ...
  Selector_Updater(session, OFF3_topBar[2], Topbar_selector1, OFF3_topBar[2])
...
OFF3_tab <- function(){
  ...
  # Topbar area
    fluidRow(box(width = 12, 
      column(width = 3, dateRangeInput(OFF3_topBar[1], label = OFF3_topBar[1])),
      column(width = 2, selectInput(OFF3_topBar[2], OFF3_topBar[2], "Unselected", selected = 1)),
      )
    ),
    # Main graph area
    fluidRow(
