...
UOF3_render <- function(output, plot1, plot2, plot3, plot4){
  output$UOF3_table_1 <- plot1
  output$UOF3_table_2 <- plot2
  output$UOF3_table_3 <- plot3
  output$UOF3_table_4 <- plot4
}

# Boolean to tell if the widgets have been loaded
UOF3_widgetsLoaded <- FALSE
...
#     goes from upper left to lower right order
UOF3_populate_Widgets <-function(session, Graph1_selector, Graph2_selector, Graph3_selector, Graph4_selector, Topbar_selector1){
  # Check in the widgets have already been loaded
  if(UOF3_widgetsLoaded){return()}
  # Populate the widgets with each of the unique values in the given data
  Selector_Updater(session, UOF3_selectors[1], Graph1_selector, UOF3_selectors[1])
  Selector_Updater(session, UOF3_selectors[2], Graph2_selector, UOF3_selectors[2])
  Selector_Updater(session, UOF3_selectors[3], Graph3_selector, UOF3_selectors[3])
  Selector_Updater(session, UOF3_selectors[4], Graph4_selector, UOF3_selectors[4])
  Selector_Updater(session, UOF3_topBar[2], Topbar_selector1, UOF3_topBar[2])
  # Mark that the widgets have been loaded
  UOF3_widgetsLoaded <<- TRUE
}
...
UOF3_tab <- function(){
  ...
  tab <- tabItem(tabName = "UOF3",
  # Topbar area
    fluidRow(box(width = 12, 
      column(width = 3, dateRangeInput(UOF3_topBar[1], label = UOF3_topBar[1])),
      column(width = 2, selectInput(UOF3_topBar[2], UOF3_topBar[2], "Unselected", selected = 1)),
      )
    ),
    # Main graph area
    fluidRow(
      ...
      tabBox(
        ...
        Plot_Maker("TAB 1", "UOF3_table_1", UOF3_selectors[1]),
        Plot_Maker("TAB 2", "UOF3_table_2", UOF3_selectors[2])
      ),
      # Makes the second graph area
      tabBox(
        height = "500px",
        # Uses functions to make what is in each tab (string is the name of the plotOutput)
        Plot_Maker("TAB 3", "UOF3_table_3", UOF3_selectors[3]),
        Plot_Maker("TAB 4", "UOF3_table_4", UOF3_selectors[4])
      )
    )
  )
  return(tab)
