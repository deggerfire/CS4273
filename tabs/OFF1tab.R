...
OFF1_render <- function(output, plot1, plot2, plot3, plot4){
  output$OFF1_table_1 <- plot1
  output$OFF1_table_2 <- plot2
  output$OFF1_table_3 <- plot3
  output$OFF1_table_4 <- plot4
}
...
#     goes from upper left to lower right order
OFF1_populate_Widgets <-function(session, Graph1_selector, Graph2_selector, Graph3_selector, Graph4_selector, Topbar_selector1){
  ...
  Selector_Updater(session, OFF1_selectors[1], Graph1_selector, OFF1_selectors[1])
  Selector_Updater(session, OFF1_selectors[2], Graph2_selector, OFF1_selectors[2])
  Selector_Updater(session, OFF1_selectors[3], Graph3_selector, OFF1_selectors[3])
  Selector_Updater(session, OFF1_selectors[4], Graph4_selector, OFF1_selectors[4])
  Selector_Updater(session, OFF1_topBar[2], Topbar_selector1, OFF1_topBar[2])
...
OFF1_tab <- function(){
  ...
  # Topbar area
    fluidRow(box(width = 12, 
      column(width = 3, dateRangeInput(OFF1_topBar[1], label = OFF1_topBar[1])),
      column(width = 2, selectInput(OFF1_topBar[2], OFF1_topBar[2], "Unselected", selected = 1)),
      )
    ),
    # Main graph area
    fluidRow(
      ...
      tabBox(
        height = "500px",
        # Uses functions to make what is in each tab (string is the name of the plotOutput)
        Plot_Maker("TAB 3", "OFF1_table_3", OFF1_selectors[3]),
        Plot_Maker("TAB 4", "OFF1_table_4", OFF1_selectors[4])
      )
    )
  )
  return(tab)
