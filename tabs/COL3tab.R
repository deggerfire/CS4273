...
#     goes from upper left to lower right order
COL3_populate_Widgets <-function(session, Graph1_selector, Graph2_selector, Graph3_selector, Graph4_selector){
...
COL3_populateTopBar <-function(session, numberOfYears)
{
  if(COL3_topBarLoaded){return()}
  Selector_Updater(session, COL3_topBar[1],numberOfYears, COL3_topBar[1])
  COL3_topBarLoaded <<- TRUE
}
...
COL3_tab <- function(){
  ...
  tab <- tabItem(tabName = "COL3",
                 ...
                 fluidRow(box(width = 12, 
                              column(width = 2, selectInput(COL3_topBar[1], COL3_topBar[1], "Unselected", selected = 1)),
                 )
                 ),
                 # Main graph area
                 fluidRow(
